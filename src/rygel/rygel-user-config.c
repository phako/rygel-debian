...
RygelUserConfig* rygel_user_config_construct (GType object_type, gboolean read_only, GError** error) {
	...
	g_debug ("rygel-user-config.vala:140: Loaded user configuration from file '%s'", path);
...
void rygel_user_config_save (RygelUserConfig* self) {
	...
	__catch0_g_file_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_critical ("rygel-user-config.vala:167: Failed to save configuration data to file '%s': %s", path, err->message);
			_g_error_free0 (err);
		}
	}
	__finally0:
